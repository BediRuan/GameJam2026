...
namespace cowsins2D
{
    public class WeaponController : MonoBehaviour, IWeaponController
    {
        ...
        [HideInInspector] public int currentWeapon { get; set; }
        ...
        {   
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (id.totalBullets >= weapon.magazineSize) // You can still reload a full magazine
                    {
                        UIController.ReduceInventoryAmount(weapon.ammoType, weapon.magazineSize - id.currentBullets);
                        id.currentBullets = weapon.magazineSize;
                    }
                    else
                    {
                        ...
                        {
                            id.currentBullets = id.currentBullets + id.totalBullets;
                            if (id.totalBullets - (weapon.magazineSize - bulletsLeft) >= 0) UIController.ReduceInventoryAmount(weapon.ammoType, weapon.magazineSize - bulletsLeft);
                            else UIController.ReduceInventoryAmount(weapon.ammoType, id.totalBullets);
                        }
                        else
                        {
                            int ToAdd = weapon.magazineSize - id.currentBullets;
                            id.currentBullets = id.currentBullets + ToAdd;
                            if (id.totalBullets - ToAdd >= 0) UIController.ReduceInventoryAmount(weapon.ammoType, ToAdd);
                            else UIController.ReduceInventoryAmount(weapon.ammoType, id.totalBullets);
                        }
                    }
                    CheckForBullets();
        ...
        public void CheckForBullets()
        {
            ...
            id.totalBullets = UIController.CheckForBullets(weapon.ammoType);
            UIController.UpdateWeaponInformation();
        }
        #endregion
