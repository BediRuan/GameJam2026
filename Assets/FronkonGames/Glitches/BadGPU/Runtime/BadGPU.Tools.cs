...
namespace FronkonGames.Glitches.BadGPU
{
  ...
  public sealed partial class BadGPU
  {
    ...
    private static readonly BadGPU[] NoEffects = new BadGPU[0];
    ...
    public static BadGPU Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is BadGPU)
              return scriptableRendererData.rendererFeatures[i] as BadGPU;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static BadGPU[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is BadGPU)
                  effects.Add(feature as BadGPU);
          }
